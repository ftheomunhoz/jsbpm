 ...
 * @return {Boolean}
 */
exports.isProcessProperty = function(localName) {
...
/**
 * @param {BPMNProcessVariable} processVariable
 */
BPMNProcessDefinition.prototype.addProcessVariable = function(processVariable) {
    if (this.variables.indexOf(processVariable) >= 0) {
        errorQueue.addError("DuplicatedVariable", {bpmnId: id, name: name, type: node.local, variable: variable}, "The process '" + name + "' already has a definition for the variable '" + variable + "'.");
    }

    this.variables.push(processVariable);
};

/**
 * @return {Array.<BPMNProcessVariable>} processVariables
 */
BPMNProcessDefinition.prototype.getProcessVariable = function(id) {
    if (id && id.length > 0) {
...
/**
 * @param {BPMNDataAssociation} dataAssociation
 */
BPMNProcessDefinition.prototype.addDataAssociation = function(dataAssociation) {
    this.dataAssociations.push(dataAssociation);
};

/**
 * @param {BPMNDataAssociation} dataAssociation
 */
BPMNProcessDefinition.prototype.addDataAssociationRef = function(ref, context) {
    var currentAssociation = this.dataAssociations[this.dataAssociations.length - 1];

    if (currentAssociation.isDataAssociationInput) { //Source is Process and Target is Task
        if (ref.isDataAssociationSrcRef) {
            currentAssociation.sourceRef = this.getProcessVariable(ref.id);
        }
        else if (ref.isDataAssociationTargetRef) {
            currentAssociation.targetRef = context.getVariable(ref.id);
        }
    } else if (currentAssociation.isDataAssociationOutput) { //Source is Task and Target is Process
        if (ref.isDataAssociationSrcRef) {
            currentAssociation.sourceRef = context.getVariable(ref.id);
        }
        else if (ref.isDataAssociationTargetRef) {
            currentAssociation.targetRef = this.getProcessVariable(ref.id);
        }
    }
};

/**
 * @return {Array.<BPMNDataAssociation>} dataAssociations
 */
BPMNProcessDefinition.prototype.getDataAssociation = function(id) {
    if (id && id.length > 0) {
        return this.dataAssociations.find(function(e) { return e.id === id;});
    }
