...
var processVariables = require('./processVariable.js');
var taskVariables = require('./taskVariable.js');
var dataAssociations = require('./dataAssociation.js');
    ...
    parser.onopentag = function (node) {
        ...
        if (inBPMN2Namespace) {
            ...
            if (currentProcessDefinition) {
                if (processVariables.isProcessVariable(localName)) {
                    currentProcessElement = processVariables.createBPMNProcessVariable(node);
                    currentProcessDefinition.addProcessVariable(currentProcessElement);

                }
                else if (tasks.isTaskName(localName)) {
                    currentProcessElement = tasks.createBPMNTask(node);
                    currentProcessDefinition.addFlowObject(currentProcessElement);

                    context = currentProcessElement;
                ...
                } else if (taskVariables.isTaskVariableDeclaration(localName)) {
                    taskVariables.addVariableToMap(node);
              
                } else if (taskVariables.isTaskVariable(localName)) {
                    currentVariable = taskVariables.createBPMNTaskVariable(node);
                    context.addVariable(currentVariable);
              
                } else if (dataAssociations.isDataAssociation(localName)) {
                    currentProcessElement = dataAssociations.createBPMNDataAssociation(node);
                    currentProcessDefinition.addDataAssociation(currentProcessElement);
              
                } else if (dataAssociations.isDataAssociationRef(localName)) {
                    currentProcessElement = dataAssociations.createBPMNDataAssociationRef(node);
                    currentProcessDefinition.addDataAssociationRef(currentProcessElement, context);
