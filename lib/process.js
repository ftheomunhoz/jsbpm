...
function BPMNProcess(id, processDefinition, eventHandler, persistency, parentProcess, parentToken, parentHistoryEntry) {
    ...
    this.variables = new BPMNProcessVariables(processDefinition);
 ...
 * @constructor
 */
var BPMNProcessVariables = function(processDefinition) {
    if (processDefinition && processDefinition.variables) {
        var obj = {};
        var currentVar = {};

        for (var i=0 ; i<processDefinition.variables.length ; i++) {
            currentVar = processDefinition.variables[i];
            obj[currentVar.name] = "";
        }
