...
var activity = require('./parsing/activity');
...
/**
 * @api {get} /:processName/:id/:activityName Get task
 * @apiName GetTask
 * @apiGroup BPMN
 *
 * @apiDescription Returns a specific task from a process instance
 *
 * @apiParam {String} processName Name of the process as used in the BPMN model
 * @apiParam {String} id Unique id of the process instance
 * @apiParam {String} id Name of the task as used in the BPMN model
 */
var getTaskRoute = '/:processName/:id/:activityName';

/**
 * @api {put} /:processName/:id/:activityName Finishes task
 * @apiName FinishTask
 * @apiGroup BPMN
 *
 * @apiDescription Finishes a specific task from a process instance
 *
 * @apiParam {String} processName Name of the process as used in the BPMN model
 * @apiParam {String} id Unique id of the process instance
 * @apiParam {String} id Name of the task as used in the BPMN model
 */
var finishTaskRoute = '/:processName/:id/:activityName';
...
exports.createServer = function(manager, options, restifyOptions) {
    ...
    server.get(getTaskRoute,
        transactionLog.transactionLoggerMiddleware({
            name: function(req){
                return 'GET ' + req.params.processName + ' FROM ' + req.params.processName + ' process: '+ req.params.id;
            }
        }),
        function(req, res, next){
            getTask(manager, req, res, next);
        });
    server.put(finishTaskRoute,
        transactionLog.transactionLoggerMiddleware({
            name: function(req){
                return 'PUT ' + req.params.processName + ' FROM ' + req.params.processName + ' process: '+ req.params.id;
            }
        }),
        function(req, res, next){
            finishTask(manager, logger, req, res, next);
        });
...
function getTask(manager, req, res, next) {
    var processId = getParameter(req, "id");
    var activityName = getParameter(req, "activityName");
    ...
    manager.get(processId, function(err, bpmnProcess){
        ...
        res.send(filterProcessTask(getProcessResponse(bpmnProcess), activityName));
    });
}

function finishTask(manager, logger, req, res, next) {
    var processId = getParameter(req, "id");
    var activityName = getParameter(req, "activityName");

    manager.get(processId, function(err, bpmnProcess){
        try {
            triggerEvent(bpmnProcess, logger, activityName + activity.activityEndHandlerPostfix, req.body);
...
function filterProcessTask(bpmnProcess, activityName) {
    var response = {};
    ...
    if (bpmnProcess) {
        response = {
            ...
            task: bpmnProcess.history.filter(function(element) { return element.name === activityName;})
        };
    }
